  ...
  return (
    <>
      <div className={classes.searchDiv}>
        <label htmlFor="select-nation">
          <h1>
            Choose a country and find out the artists ruling the charts over
            there!
          </h1>
        </label>
        <select name="" id="select-nation" onChange={changeCountry}>
          {nationsArr.map((nation) => (
            <option value={nation.id} key={nation.id}>
              {nation.title}
            </option>
          ))}
        </select>
      ...
      {isDataLoaded === false && (
        <div className={classes.styleSpinner}>
          <TailSpin
            height="80"
            width="80"
            color="#4fa94d"
